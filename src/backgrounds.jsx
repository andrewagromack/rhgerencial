...
function BGGrid({ speed = 1, color = '#c9a961' }) {
  return (
    <div className="bg-grid">
      ...
      `}</style>
      <svg className="grid-svg" preserveAspectRatio="none">
        <defs>
          <pattern id="bg-grid-p" width="50" height="50" patternUnits="userSpaceOnUse">
            <path d="M 50 0 L 0 0 0 50" fill="none" stroke={color} strokeWidth="0.5" opacity="0.5" />
          </pattern>
          <radialGradient id="bg-grid-mask" cx="50%" cy="50%" r="65%">
            <stop offset="0%"  stopColor="#000" stopOpacity="0" />
            <stop offset="100%" stopColor="#000" stopOpacity="1" />
          </radialGradient>
        </defs>
        <rect width="100%" height="100%" fill="url(#bg-grid-p)" />
        <rect width="100%" height="100%" fill="url(#bg-grid-mask)" />
      </svg>
      <div className="scan" />
      {[
        { top: '20%', left: '15%', delay: 0 },
        { top: '35%', left: '78%', delay: 0.6 },
        { top: '62%', left: '22%', delay: 1.2 },
        { top: '48%', left: '55%', delay: 1.8 },
        { top: '78%', left: '70%', delay: 2.4 },
        { top: '12%', left: '60%', delay: 0.3 },
      ].map((n, i) => (
        <div key={i} className="node" style={{ top: n.top, left: n.left, animationDelay: `${n.delay}s` }} />
      ))}
    </div>
  );
}
...
function BGCircuit({ speed = 1, color = '#c9a961' }) {
  const ref = useRef2(null);
  useEffect2(() => {
    const canvas = ref.current; if (!canvas) return;
    const ctx = canvas.getContext('2d');
    let raf, w, h, nodes = [], pulses = [];
    ...
    const tick = (ts) => {
      ...
      if (!lastEmit || ts - lastEmit > 350 / speed) {
        emitPulse(); lastEmit = ts;
      }
      pulses = pulses.filter(p => p.t < p.life);
      for (const p of pulses) {
        p.t += 1;
        const k = p.t / p.life;
        const mx = p.a.x, my = p.b.y; // codo manhattan
        let px, py;
        if (k < 0.5) {
          const kk = k * 2;
...
function BGStack({ speed = 1, color = '#c9a961' }) {
  ...
  return (
    <div className="bg-stack">
      ...
      `}</style>
      {/* Reglas horizontales (estructura) */}
      {Array.from({ length: 8 }).map((_, i) => (
        <div key={`r${i}`} className="rule" style={{ top: `${(i + 1) * 11}%` }} />
      ))}
      {/* Ticks verticales */}
      {Array.from({ length: 10 }).map((_, i) => (
        <div key={`t${i}`} className="tick" style={{ left: `${(i + 1) * 9}%` }} />
      ))}
      {/* Barras barriendo */}
      {bars.map((b, i) => (
...
function BGBeam({ speed = 1, color = '#c9a961' }) {
  ...
  return (
    <div className="bg-beam">
      ...
      {Array.from({ length: 18 }).map((_, i) => (
        <div key={`c${i}`} className="col" style={{ left: `${(i / 18) * 100}%` }} />
      ))}
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={`h${i}`} className="hline" style={{ top: `${(i + 1) * 14}%` }} />
      ))}
      {beams.map((b, i) => (
...
// ====== Selector ======
function Background({ variant, speed, color }) {
  const map = {
    grid:    BGGrid,
    circuit: BGCircuit,
    stack:   BGStack,
  ...
  return <Comp speed={speed} color={color} />;
}
